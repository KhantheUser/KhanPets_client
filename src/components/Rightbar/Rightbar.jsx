...
function Rightbar() {
  ...
  const handleSend = async()=>{
      ...
      socket.current.emit('sendMessage',{receiverId : currentChatUser.id,conversationId})
  ...
  return (
    <div className='rightBar'>
      <div className="rightbarWrapper">
        <div className="top">
          <div className="image" >
            <img src={`${currentChatUser?.avatar ? currentChatUser?.avatar : '/assets/images/defaultavatar.jpg'}`} alt="" />
          </div>
          <div className="name">{currentChatUser?.username}</div>
        </div>
        <div className="chat" >
            
            {messages?.map((message,index)=><Message key={index} message={message} own={currentUser._id === message.sender._id}/>)}
        </div>
